...
{
  m_ParticlePool.resize(PARTICLE_MAX_COUNT);
}
...
void ParticleSystem::OnRender(GLCore::Utils::OrthographicCamera &camera)
{
  if (!m_QuadVA)
  {
    ...
    m_ParticleShader = std::unique_ptr<GLCore::Utils::Shader>(GLCore::Utils::Shader::FromGLSLTextFiles(
        "assets/shaders/particle.glsl.vert",
        "assets/shaders/particle.glsl.frag"));
